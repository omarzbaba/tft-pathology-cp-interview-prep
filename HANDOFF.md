...
- **The study DASHBOARD is now BUILT** — a single self-contained `index.html` (generated by `node dashboard/build.mjs` from the markdown corpus) served via GitHub Pages at https://omarzbaba.github.io/tft-pathology-cp-interview-prep/ with access code `TFT`. Source lives in `dashboard/` (`build.mjs`, `app.css`, `app.js`, `template.html`). To change content, edit the markdown in `content/` and re-run the build.
